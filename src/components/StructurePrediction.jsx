...
class StructurePrediction extends React.Component {
    ...
    findRanges = (array) => {
        ...
        let ranges = [
            { start_residue_number: array[0], end_residue_number: array[0] },
        ];
        ...
        for (let i = 1; i < array.length; i++) {
            ...
            if (array[i] <= currentRange.end_residue_number + 1) {
                ...
                ranges.push({
                    start_residue_number: array[i],
                    end_residue_number: array[i],
                });
            }
        }
        return ranges;
    ...
    componentWillReceiveProps(newProps) {
        ...
        if (newProps.data !== null) {
            this.reDraw3D(
                newProps.data,
                newProps.featureSelection.selectionStart,
                newProps.featureSelection.selectionEnd
            );
        }
    ...
    addAnnotations() {
        if (this.annotations.predictedTransmembrane) {
            ...
            this.annotations.predictedTransmembrane = [
                ...this.findRanges(toplogy["."]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["predictedTransmembrane"].contrast[
                            "."
                        ];
                    return e;
                }),
        ...
        if (this.annotations.predictedConservation) {
            let conservation = this.findIndexes(
            ...
            this.annotations.predictedConservation = [
                ...this.findRanges(conservation["0"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["0"];
                    return e;
                }),
                ...this.findRanges(conservation["1"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["1"];
                    return e;
                }),
                ...this.findRanges(conservation["2"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["2"];
                    return e;
                }),
                ...this.findRanges(conservation["3"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["3"];
                    return e;
                }),
                ...this.findRanges(conservation["4"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["4"];
                    return e;
                }),
                ...this.findRanges(conservation["5"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["5"];
                    return e;
                }),
                ...this.findRanges(conservation["6"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["6"];
                    return e;
                }),
                ...this.findRanges(conservation["7"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["7"];
                    return e;
                }),
                ...this.findRanges(conservation["8"]).map((e) => {
                    e["color"] =
                        proteinColorSchemes["conservation"].contrast["8"];
                    return e;
                }),
            ];
        }
        ...
        if (this.annotations.predictedBindingNucleicAcids) {
            let bindingNucleic = this.findIndexes(
        ...
        if (this.annotations.predictedBindingSmallMolecules) {
            let bindingSmall = this.findIndexes(
    ...
    render() {
        return (
            <>
                ...
                {(this.annotations?.predictedDisorder !== undefined ||
                    this.annotations?.predictedDSSP3 !== undefined ||
                    this.annotations?.predictedTransmembrane !== undefined) && (
                    <div>
                        <p>
                            <h2>
                                👇 Click to color structure by predictions of:
                            </h2>{" "}
                            <ul>
                                {this.annotations?.predictedDSSP3 && (
                                    <li
                                ...
                                {this.annotations?.predictedTransmembrane && (
                                    <li
                                        ...
                                        onClick={() =>
                                            this.overlayAnnotations("topology")
                                        }
                                    >
                                        Topology (strand{" "}
                                        <span
                                ...
                                {this.annotations
                                    ...
                                    <li
                                        ...
                                        onClick={() =>
                                            this.overlayAnnotations("nucleic")
                                        }
                                    >
                                        <span
                                            style={{
                                ...
                                {this.annotations?.predictedDisorder && (
                                    <li
                                        ...
                                        onClick={() =>
                                            this.overlayAnnotations("disorder")
                                        }
                                    >
                                        <span
                                            style={{
